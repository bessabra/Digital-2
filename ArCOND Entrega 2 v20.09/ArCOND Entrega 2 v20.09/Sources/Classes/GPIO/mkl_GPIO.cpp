...
void mkl_GPIO2::setPortMode(gpio_PortMode mode) {
  if (mode == gpio_input) {
    *addressPDDR &= ~pinPort;
  } else {
    *addressPDDR |= pinPort;
 ...
 */
void mkl_GPIO2::setPullResistor(gpio_PullResistor pull) {
 ...
 */
void mkl_GPIO2::writeBit(int bit) {
  if (bit) {
    *addressPDOR |= pinPort;
  } else {
    *addressPDOR &= ~pinPort;
 ...
 *             - PortxPCRn: Pin Control Register.P�g. 183 (Mux) and 185 (Pull).
 */
void mkl_GPIO2::bindPeripheral(uint8_t GPIONumber, uint8_t pinNumber) {
 ...
 */
void mkl_GPIO2::enableModuleClock(uint8_t GPIONumber) {
 ...
 *             - PortxPCRn: Pin Control Register.P�g. 183 (Mux) and 185 (Pull).
 */
void mkl_GPIO2::selectMuxAlternative() {
 ...
 *
 */
void mkl_GPIO2::setGPIOParameters(gpio_Pin pin, uint32_t &gpio,
                                     uint32_t &pinNumber) {
  pinNumber = pin & 0xFF;
  gpio = pin >> 8;
  pinPort = 1 << pin;
